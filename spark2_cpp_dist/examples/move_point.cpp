...
int main(int argc, char *argv[]){ 
    ...
    prefix_text = "==> moveToolPoint (position: (" + 
                std::to_string(target_pose.position.x) + ", " + 
                std::to_string(target_pose.position.y) + ", " + 
                std::to_string(target_pose.position.z) + "), orientation: (" + 
                std::to_string(target_pose.orientation.w) + ", " + 
                std::to_string(target_pose.orientation.x) + ", " + 
                std::to_string(target_pose.orientation.y) + ", " + 
                std::to_string(target_pose.orientation.z) + "))\n";
    robot.moveToolPoint(target_pose, v, 0);
    printFeedback(robot, print_dt, timeout, &current_joint_pos, &current_pose, prefix_text);
    ...
    target_pose.position.z -= 0.05;
    target_pose.position.y += 0.1;
    prefix_text = "==> moveToolLine (position: (" + 
                std::to_string(target_pose.position.x) + ", " + 
                std::to_string(target_pose.position.y) + ", " + 
                std::to_string(target_pose.position.z) + "), orientation: (" + 
                std::to_string(target_pose.orientation.w) + ", " + 
                ...
                std::to_string(target_pose.orientation.y) + ", " + 
                std::to_string(target_pose.orientation.z) + "))\n";
    robot.moveToolLine(target_pose, 0, t);
    printFeedback(robot, print_dt, timeout, &current_joint_pos, &current_pose, prefix_text);
