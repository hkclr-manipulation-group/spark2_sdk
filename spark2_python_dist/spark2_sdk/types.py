...
def _is_numpy_array(obj: Any) -> bool:
    """Return True if obj is a numpy ndarray and numpy is installed."""
...
class DiagnosticFlags:
    ...
    FAULT_ARM_SIZE_MISMATCH = 1 << 25
    FAULT_GRIPPER_SIZE_MISMATCH = 1 << 26
    FAULT_ARM_JOINT_SIZE_MISMATCH = 1 << 27
    FAULT_GRIPPER_JOINT_SIZE_MISMATCH = 1 << 28
    FAULT_ROBOT_NAME_MISMATCH = 1 << 29

    FAULT_COLLISION_DETECTED = 1 << 30
    FAULT_RECOVERY_REQUIRED = 1 << 31
    FAULT_UNKNOWN = 1 << 31  # legacy alias

    INVALID_CONTROL_TYPE_COMBINATION = 1 << 32
    CARTESIAN_CONTROL_REQUIRE_POSITION_TARGET = 1 << 33
    CONTROL_STRATEGY_NOT_AVAILABLE = 1 << 34
    WAYPOINT_CONTROL_STRATEGY_NOT_ALLOWED = 1 << 35
    WAYPOINT_TARGET_TYPE_NOT_ALLOWED = 1 << 36
    WAYPOINT_SMOOTHING_METHOD_NOT_ALLOWED = 1 << 37
    PLAYBACK_CONTROL_REQUIRE_POSITION_TARGET = 1 << 38
    PLAYBACK_CONTROL_START_POSE_NOT_REACHABLE = 1 << 39
...
class BaseJointState:
    ...
    def __init__(self, data: Union[List, np.ndarray, tuple, Any], *args) -> None:
        ...
        elif isinstance(data, (list, tuple)) or _is_numpy_array(data):
            sequence = data
        else:
            sequence = [data]
